...
namespace CoreLib.Submodule.LootDrop.Patches
{
    /// <summary>
                ...
                try
                {
                    if (DropTablesModule.DropTableModification.ContainsKey(lootTable.id))
                    {
                        DropTableModificationData modificationData =
                            DropTablesModule.DropTableModification[lootTable.id];
                        if (lootTable.lootInfos != null && lootTable.guaranteedLootInfos != null)
                        {
                            DropTablesModule.RemoveDrops(lootTable.lootInfos, lootTable.guaranteedLootInfos,
                                modificationData);
                            DropTablesModule.EditDrops(lootTable, lootTable.lootInfos, lootTable.guaranteedLootInfos,
                                modificationData);
                            DropTablesModule.AddDrops(lootTable, lootTable.lootInfos, lootTable.guaranteedLootInfos,
                                modificationData);
                        }
                    }
                }
