...
namespace PugMod
{
    public partial class ModSDKWindow
    {
        private class UploadMod
        {
            ...
            public void Refresh()
            {
                if (!EditorPrefs.HasKey(CHOSEN_MOD_KEY)) return;
            ...
            public void OnEnable(VisualElement root)
            {
                ...
                _modIOSettings = new List<ModSettings>(AssetDatabase.FindAssets("t:PugMod.ModIO.ModSettings")
                    ...
                    .Select(AssetDatabase.LoadAssetAtPath<ModSettings>));
                ...
                _nameTextField.RegisterValueChangedCallback(evt =>
                {
                    ...
                    while (_modSettings.FindIndex(x => x.metadata.name.Equals(newName) && x != modBuilderSettings) !=
                           -1)
                    {
                ...
                _modLogoButton.clicked += () =>
                {
                    if (!GetModSettings(out var modBuilderSettings, out var modIOSettings,
                            createModIOSettingsIfMissing: true))
                    {
                        return;
                    }
                ...
                _modLogoButton.RegisterCallback<MouseEnterEvent>(_ =>
                {
                    _modLogo.style.unityBackgroundImageTintColor = new StyleColor(new Color(1, 1, 1, 0.33f));
                });

                _modLogoButton.RegisterCallback<MouseLeaveEvent>(_ =>
                {
                ...
                {
                    GetModSettings(out var modBuilderSettings, out var modIOSettings,
                        createModIOSettingsIfMissing: true);
                    ...
                    ModIOUnity.CreateModProfile(creationToken, modProfileDetails, result =>
                    {
                        ...
                        EditorUtility.DisplayDialog("Register successful",
                            $"Successfully registered new mod with ID={modIOSettings.modId}", "OK");
                    });
                };
                ...
                _uploadButton.clicked += () =>
                {
                    ...
                    ModIOUnity.GetMod(new ModId(modIOSettings.modId), result =>
                    {
                        ...
                        ModIOUnity.EditModProfile(modProfileDetails, result1 =>
                        {
                ...
                {
                    if (!GetModSettings(out _, out var modIOSettings) || modIOSettings == null)
                    {
                        return;
                    }
                ...
                _modList.RegisterValueChangedCallback(_ => UpdateSelection());
            }
            ...
            private void UpdateModIOSettings(ModSettings modIOSettings)
            {
                ...
                if (modIOSettings.logo != _modLogo.style.backgroundImage.value.texture &&
                    _modLogo.style.backgroundImage.value.texture is not null)
                {
            ...
            {
                if (modBuilderSettings is null || modIOSettings is null)
                {
                ...
                try
                {
                    ...
                    if (dir != null)
                        AssetDatabase.CreateAsset(settings,
                            Path.Combine(dir, $"{modBuilderSettings.metadata.name}_modio.asset"));
            ...
            private bool GetModSettings(out ModBuilderSettings modBuilderSettings, out ModSettings modIOSettings,
                string modName = null, bool createModIOSettingsIfMissing = false)
            {
                if (modName == null)
                {
            ...
            private void UpdateSelection()
            {
                ...
                ModIOUnity.GetMod(new ModId(modIOSettings.modId), result =>
                {
                    ...
                    {
                        Debug.LogError(
                            $"failed to fetch mod info for mod with ID {modIOSettings.modId}: {result.result.message} (code: {result.result.errorCode}");
                        return;
                    }

                    if (!GetModSettings(out modBuilderSettings, out var currentModIOSettings) ||
                        currentModIOSettings != modIOSettings)
                    {
                        return;
                    }
                    ...
                    if (!string.IsNullOrEmpty(result.value.logoImage_Original.filename))
                    {
                        try
                        {
                            ...
                            {
                                if (!downloadTextureResult.result.Succeeded()) return;

                                modIOSettings.logo = downloadTextureResult.value;
                                if (modIOSettings.logo is not null) _modLogo.style.backgroundImage = modIOSettings.logo;
                            });
                        }
                        catch (Exception e)
                        {
