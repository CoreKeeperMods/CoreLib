...
public sealed class ThreadingHelper : MonoBehaviour, ISynchronizeInvoke
{
    ...
    public ThreadingHelper(IntPtr pointer) : base(pointer)
    {
    }
    ...
    {
        logSource = new ManualLogSource("CoreLib.ThreadingHelper");
        Instance = CoreLibPlugin.Instance.AddComponent<ThreadingHelper>();
    }
    ...
    {
        logSource.Log(LogLevel.Error, ex);
        if (ex.InnerException != null) logSource.Log(LogLevel.Error, $"INNER: {ex.InnerException}");
    }
