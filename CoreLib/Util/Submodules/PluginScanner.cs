            ...
            get {
                if (_pluginsAssemblyDefinitions == null) {
                    ...
                    foreach (string dll in Directory.GetFiles(Paths.PluginPath, "*.dll", SearchOption.AllDirectories)) {
                        ...
                        if (fileName.ToLower().Contains("corelib") || fileName.ToLower().Contains("mmhook")) {
                            continue;
                        }
                        ...
                        try {
                            ...
                            CoreLibPlugin.Logger.LogDebug($"Cecil ReadAssembly couldn't read {dll}");
                        }
                    }
            ...
            foreach (var assemblyDef in assemblies) {
                ...
                foreach (var otherAssemblyDef in assemblies) {
                    ...
                    if (oldDuplicateAssembly != null) {
                        CoreLibPlugin.Logger.LogDebug($"Removing {oldDuplicateAssembly.MainModule.FileName} (ModVer : {oldDuplicateModVer}) from the list " +
