...
namespace PugMod
{
	public class ImporterSettings : ScriptableObject
	{
		...
		public string assetRipperEquipmentSlotPrefabsPath = "ExportedProject/Assets/GameObject";
		...
		private void Initialize()
		{
			...
			if (initializedToVersion < 4)
			{
				//SpriteInstancing has been renamed to PugSprite.dll
				excludeGameAssemblies.Add("PugSprite.dll");
				excludeGameAssemblies.Add("ScriptableData.dll");
			}

			// Included in SDK project instead
			if (initializedToVersion < 5)
			{
				includeGameAssemblies.Remove("NaughtyAttributes.Core.dll");
				excludeGameAssemblies.Add("^NaughtyAttributes");
			}

			if (initializedToVersion < 6)
			{
				...
				includeGameAssemblies.Add("steam_api64.dll");
			}
