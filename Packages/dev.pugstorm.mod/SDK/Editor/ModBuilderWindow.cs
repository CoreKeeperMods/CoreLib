	...
	public class ModBuilderWindow : EditorWindow
	{
		...
		public static ModBuilderSettings CreateNewMod(string modName)
		{
			...
			try
			{
				...
				var contextNumber = ScriptableDataEditorUtility.AddContext(Path.Combine(newModDirectory, "Data"), modName);
				var scriptableDataDirectoryAsset = ScriptableDataEditorUtility.GetContext(contextNumber).directoryInfo;
				scriptableDataDirectoryAsset.enableOverloading = true;
				EditorUtility.SetDirty(scriptableDataDirectoryAsset);
				AssetDatabase.SaveAssetIfDirty(scriptableDataDirectoryAsset);
